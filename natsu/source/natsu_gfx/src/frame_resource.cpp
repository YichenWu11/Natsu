...
namespace Natsu::GFX {
    FrameResource::FrameResource(UINT64 cpuFence, ID3D12Fence* gpuFence, ID3D12Device* device) :
        ...
        cmdList.Create(device, cmdAllocator.Get());
    ...
    void FrameResource::Execute(CommandQueue& queue) {
        if (!populated) return;
        queue.Execute(cmdList.GetCommandList());
    }

    void FrameResource::Signal(CommandQueue& cmdQueue, UINT64 cpuFence) {
    ...
    CmdListHandle FrameResource::Command() {
        populated = true;
        return {cmdAllocator.Get(), cmdList.GetCommandList()};
    }
    ...
    void FrameResource::DrawMesh(
        ...
        PropertyBinder binder{
            .cmdList = cmdList.GetCommandList(),
            .shader  = shader};
        ...
        cmdList.DrawIndexed(mesh->IndexBuffer().GetByteSize() / 4, 0, 0);
    }
